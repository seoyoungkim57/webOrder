...
export default function DestinationsPage() {
  ...
  const fetchDestinations = async (searchQuery?: string) => {
    try {
      const url = searchQuery
        ? `/api/destinations?search=${encodeURIComponent(searchQuery)}`
        : '/api/destinations'
      ...
      console.error('수주자 목록 조회 실패:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (id: string, nickname: string) => {
    if (!confirm(`"${nickname}" 거래처를 삭제하시겠습니까?`)) return
          ...
          <div className="p-6 border-b">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center px-4 py-2 bg-purple-600 text-white rounded-md text-sm hover:bg-purple-700 transition-colors"
              >
            ...
            {destinations.length === 0 ? (
              <div className="p-12 text-center text-gray-500">
                {search ? (
                  ...
                  <>
                    <p className="mb-4">등록된 거래처가 없습니다.</p>
                    <Link
                      href="/destinations/new"
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-500 mt-1">
                        ...
                        {destination.phone2 && ` / ${destination.phone2}`}
                      </div>
                      <div className="text-sm text-gray-400 mt-1">
                        {destination.address}
                        {destination.addressDetail && ` ${destination.addressDetail}`}
                      </div>
                      {destination.usageCount > 0 && (
                        <div className="text-xs text-gray-400 mt-2">
                          {destination.usageCount}회 사용
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <button
                        onClick={() => handleDelete(destination.id, destination.nickname)}
                      ...
                      >
                        {deleting === destination.id ? '삭제 중...' : '삭제'}
                      </button>
                    </div>
                  </div>
                </div>
